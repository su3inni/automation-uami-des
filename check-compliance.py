...
def run():
    ...
    for sub_id in WORKLOAD_SUBSCRIPTIONS:
        # workload subscription's compute / all resource
        compute = ComputeManagementClient(cred, sub_id)
        resource = ResourceManagementClient(cred, sub_id)

        # des in workload subscription
        for des in compute.disk_encryption_sets.list_by_subscription():
            des_name = des.name
            ...
            # UAMI 없으면 다음
            if len(uamis)==0 : break 

            # linked resource w/des 
            ...
            for uami_id in uamis :  
                ...
                for res in linked_resources or [None]:  # 리소스 없으면 None 처리
                    if res:
